...
Use this to switch a channel between 'base' and 'devtools' container images, or to update the Claude model.`,
  {
    jid: z.string().describe('Channel JID to update'),
    image: z
      .string()
      .optional()
      .describe('Container image tag: "base" or "devtools"'),
    model: z
      .string()
      .optional()
      .describe('Claude model override (e.g., "opus", "sonnet", "haiku")'),
  },
  async (args) => {
    if (!isMain) {
      return {
        content: [
          {
            type: 'text' as const,
            text: 'Only the admin channel can update channel configurations.',
          },
        ],
        isError: true,
      };
    ...
    const changes = [];
    if (args.image) changes.push(`image: ${args.image}`);
    ...
    return {
      content: [
        {
          type: 'text' as const,
          text: `Channel configuration updated (${changes.join(', ')}). Changes will take effect on next container restart.`,
        },
      ],
    };
  },
