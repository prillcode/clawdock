...
ClawDock can route agent API calls through an alternative provider instead of Anthropic directly. This is useful for cost savings, higher rate limits, or accessing different models through a compatible API proxy.

## How It Works

ClawDock containers run the Claude Agent SDK, which makes API calls to Anthropic by default. When provider override env vars are set in `.env`, the container-runner forwards them to the container instead of the default `CLAUDE_CODE_OAUTH_TOKEN`. The Claude CLI inside the container uses `ANTHROPIC_BASE_URL` to determine where to send requests.
...
When `ANTHROPIC_BASE_URL` is set, `CLAUDE_CODE_OAUTH_TOKEN` is **excluded** from the container — provider's auth token takes precedence automatically.

## Setup

### 1. Configure `.env`

Comment out (or remove) Anthropic OAuth token and set provider override vars:
...
# Build and restart
npm run build && systemctl --user restart clawdock
```
...
Comment out provider vars and uncomment `CLAUDE_CODE_OAUTH_TOKEN`:
...
These are forwarded to the container alongside provider auth vars. The Claude CLI uses them to override its internal model resolution.
...
ClawDock supports different Claude model aliases per Discord channel, configured in `.env`:
...
These aliases are stored in the database and passed to Claude Agent SDK's `query()` call. When combined with a provider override, the alias resolution chain is:
...
Any provider that implements Anthropic API format and accepts `ANTHROPIC_BASE_URL` should work. Known compatible providers:
...
ClawDock is a fork of NanoClaw and maintains code references to `nanoclaw` for:
- **Upstream merge compatibility** — Internal code paths use "nanoclaw" to minimize conflicts when pulling updates from NanoClaw upstream
- **Simplicity** — Avoids changing configuration paths and container names in code

All references to "nanoclaw" in paths (e.g., `~/.config/nanoclaw/`, docker container names, service file) are intentional and should remain unchanged. Only branding in documentation and user-facing messages has been updated to "ClawDock".
...
- The mount allowlist (`~/.config/nanoclaw/mount-allowlist.json`) is stored outside of project root to prevent container tampering
